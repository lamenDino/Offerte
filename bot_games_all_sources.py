...
class FreeGamesBot:
    ...
    async def fetch_prime(self):
        ...
        try:
            ...
            games=soup.find_all("div",class_="loot-item")  # verifica la classe
            for g in games:
                title_el=g.find("h3")
    ...
    async def fetch_gog(self):
        ...
        try:
            r=requests.get(url,headers=headers,timeout=15)
            soup=BeautifulSoup(r.text,"html.parser")
            # esempio: titolo in h3 con classe "product-title", link nel padre <a>
    ...
    async def fetch_bnet(self):
        # Placeholder da implementare scraping autenticato Battle.net
        return []

    async def fetch_riot(self):
        # Placeholder da implementare scraping eventi Riot Games
        return []
